...
class TestMoveLog(unittest.TestCase):
    ...
    def test_a_landed_move_is_recorded_with_no_captures(self):
        arbiter, board, rule_engine = make_arbiter(rules={"R": allow_rule()})
        piece = make_piece("w", "R", 0, 0)
        board.add_piece(piece)
        arbiter.schedule(translate_motion(piece, Position(0, 0), Position(0, 1), start_time=0, duration=1000))
        ...
        self.assertTrue(entry["source"].equals(Position(0, 0)))
        self.assertTrue(entry["destination"].equals(Position(0, 1)))
        self.assertEqual(entry["captured"], [])

    def test_a_capturing_move_records_the_captured_kind(self):
        arbiter, board, rule_engine = make_arbiter(rules={"R": allow_rule()})
        mover = make_piece("w", "R", 0, 0)
        enemy = make_piece("b", "P", 0, 1)
        board.add_piece(mover)
        ...
        self.assertEqual(arbiter.move_log[0]["captured"], ["P"])

    def test_a_bounce_back_to_the_legs_own_source_is_not_logged_as_a_move(self):
        deny_rule = Mock()
        deny_rule.is_legal.return_value = False
        arbiter, board, rule_engine = make_arbiter(rules={"R": deny_rule})
        piece = make_piece("w", "R", 0, 0)
        ...
        self.assertEqual(arbiter.move_log, [])

    def test_a_multi_leg_route_logs_the_whole_route_as_one_move(self):
        arbiter, board, rule_engine = make_arbiter(cols=4, rules={"R": allow_rule()})
        piece = make_piece("w", "R", 0, 0)
        board.add_piece(piece)
        motion = translate_motion(piece, Position(0, 0), Position(0, 1), start_time=0, duration=1000,
        ...
        entry = arbiter.move_log[0]
        self.assertTrue(entry["source"].equals(Position(0, 0)))
        self.assertTrue(entry["destination"].equals(Position(0, 2)))

    def test_captures_along_a_multi_leg_route_are_all_attributed_to_the_one_move(self):
        # A static occupant always lands the mover right there (real chess
        # capture semantics) - a route only continues past a capture when
        # the captured piece was itself mid-flight (a race), so this needs
        # one in-flight capture (leg 1) followed by a static one at the
        # final landing cell (leg 2) to get two captures in a single move.
        arbiter, board, rule_engine = make_arbiter(cols=4, rules={"R": allow_rule()})
        mover = make_piece("w", "R", 0, 0)
        racing_enemy = make_piece("b", "R", 0, 1)
        final_enemy = make_piece("b", "N", 0, 2)
        board.add_piece(mover)
        board.add_piece(racing_enemy)
        board.add_piece(final_enemy)
        racing_enemy_motion = translate_motion(racing_enemy, Position(0, 1), Position(0, 3),
                                                start_time=0, duration=1000, sequence=2)
        mover_motion = translate_motion(mover, Position(0, 0), Position(0, 1), start_time=0, duration=1000,
                                         sequence=1, remaining_route=[Position(0, 2)])
        arbiter.schedule(mover_motion)
        arbiter.schedule(racing_enemy_motion)

        arbiter.advance(2000)

        self.assertEqual(board.get_piece(Position(0, 2)), mover)
        self.assertEqual(len(arbiter.move_log), 1)
        self.assertEqual(sorted(arbiter.move_log[0]["captured"]), ["N", "R"])


class TestGetScore(unittest.TestCase):

    def test_no_captures_means_zero_for_both_colors(self):
        arbiter, board, rule_engine = make_arbiter(rules={"R": allow_rule()})
        piece = make_piece("w", "R", 0, 0)
        board.add_piece(piece)
        arbiter.schedule(translate_motion(piece, Position(0, 0), Position(0, 1), start_time=0, duration=1000))
        ...
        self.assertEqual(arbiter.get_score(), {"w": 0, "b": 0})

    def test_a_capture_credits_the_capturing_colors_material_value(self):
        arbiter, board, rule_engine = make_arbiter(rules={"R": allow_rule()})
        mover = make_piece("w", "R", 0, 0)
        enemy_rook = make_piece("b", "R", 0, 1)
        board.add_piece(mover)
        ...
        self.assertEqual(arbiter.get_score(), {"w": 5, "b": 0})

    def test_captures_by_both_colors_accumulate_independently(self):
        arbiter, board, rule_engine = make_arbiter(cols=4, rules={"R": allow_rule()})
        white_mover = make_piece("w", "R", 0, 0)
        black_pawn = make_piece("b", "P", 0, 1)
        black_mover = make_piece("b", "R", 2, 0)
        white_pawn = make_piece("w", "P", 2, 1)
        board.add_piece(white_mover)
        board.add_piece(black_pawn)
        board.add_piece(black_mover)
        board.add_piece(white_pawn)
        arbiter.schedule(translate_motion(white_mover, Position(0, 0), Position(0, 1), start_time=0, duration=1000, sequence=1))
        arbiter.schedule(translate_motion(black_mover, Position(2, 0), Position(2, 1), start_time=0, duration=1000, sequence=2))

        arbiter.advance(1000)

        self.assertEqual(arbiter.get_score(), {"w": 1, "b": 1})
